...
h2. So... how did we get to be so many already then?
...
It's quite simple:
# Mr X (a candidate) approaches Mrs Y (a Bestbrainer) and asks to join {{ site.company_name }}.
# Y does what it takes to evaluate X. Start by finding out which {{ site.company_members }} have worked with Mr X before and that they think of him. Then organize interviews and tests and whatever else she deems necessary. She might invite X to join the next "hack summit":hack-summit.html.
# If Y thinks X is suitable, she emails all {{ site.company_members }}: "I think X should be a Bestbrainer, because [reasons].  Anyone against?".
